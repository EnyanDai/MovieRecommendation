...
matrix: $(BIN)PearsonsCorrelation.class
	...
	time java -cp .:$(BIN) PearsonsCorrelation -trainingFile data/ra.train -outputFile /tmp/r0685971.matrix

predict: $(BIN)MovieRunner.class
	@echo "Testing the prediction of movie ratings on the full dataset"
	time java -cp .:$(BIN) MovieRunner -trainingFile data/ra.train -testFile data/ra.test -matrixFile /tmp/r0685971.matrix
